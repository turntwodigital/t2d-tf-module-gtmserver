resource "google_certificate_manager_certificate" "sgtm_ssl_cert" {
  ...
  name       = "${var.resource_prefix}-sgtm-ssl-cert-${replace(each.key, ".", "-")}"
  managed {
    domains = [each.key]
  }
}
